...
Raw third-party data are not redistributed. `DATA_SOURCES.md` lists the permanent source, release or access version, licence, and checksum for every input. Processed tables whose source terms permit redistribution are included in the Zenodo archive. The Oxford processed tables are excluded because the acquisition record did not provide an explicit redistribution licence.
...
The full rebuild is CPU intensive and can take several hours. Every experiment writes a resolved configuration, input and source hashes, checkpoint metadata, result tables, and an audit record. `REPRODUCIBILITY.md` describes the verification layers and expected outputs.
